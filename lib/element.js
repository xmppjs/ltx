 ...
 * preventEscape is a boolean to allow xml
 * passed as text nodes to be preserved
 **/
function Element(name, attrs, preventEscape) {
    this.name = name;
    this.parent = null;
    this.attrs = attrs || {};
    this.children = [];
    this.preventEscape = preventEscape;
}
...
Element.prototype.clone = function() {
    var clone = new Element(this.name, {}, this.preventEscape);
...
Element.prototype.write = function(writer) {
	...
	for(var i = 0; i < this.children.length; i++) {
		...
		if (child.write)
			...
			writer(escapeXmlText(child, this.preventEscape));
		else if (child.toString)
			writer(escapeXmlText(child.toString(), this.preventEscape));
	    }
        }
        writer("</");
        writer(this.name);
...
function escapeXml(s, preventEscape) {
    if (preventEscape) {
        return s;
    }
...
function escapeXmlText(s, preventEscape) {
    if (preventEscape) {
        return s;
    }
